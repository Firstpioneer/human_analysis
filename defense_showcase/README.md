...
这张流程图适合用于展示系统不是单点工具，而是从招聘需求、候选人证据、成长性推理、动态面试到评估复盘的完整闭环。重点可以放在五类数据如何汇合到面试引擎：岗位画像提供“要考察什么”，需求场景提供“为什么优先考察这些维度”，简历解析提供“候选人声称和经历了什么”，成长性推理提供“未来发展潜力如何解释”，外部数字足迹提供“是否存在可验证证据”。
...
```mermaid
flowchart TB
    U[浏览器 SPA] --> R[FastAPI Routers]
    R --> S[Services 业务服务层]
    S --> C[Converters 数据转换层]
    S --> L[LLM / OCR / GitHub / 语音服务]
    C --> ST[Storage JSON 存储层]
    S --> ST
    ST --> D[(data/ 本地数据)]
```
...
- 前端负责页面状态、用户交互、文件上传、面试房间和记录展示。
- 路由层负责 HTTP 入参校验、异常返回和服务编排入口。
- 服务层负责画像对话、简历解析、问题生成、追问和评价等核心业务。
- 转换层负责把画像、简历结果转换为面试模块可消费的数据结构。
- 存储层负责本地 JSON 文件读写，保证原型系统不依赖外部数据库也能完整运行。

这种分层让系统在展示时更容易说明“为什么能扩展”：如果后续要接数据库，主要替换 storage；如果要接新的模型供应商，主要替换 LLM 服务；如果要接企业 ATS，主要扩展 routers 和 converters。
...
- `multidimensional_profile`：根据履历证据生成职业倾向坐标，例如技术深耕/业务管理、独立产出/协作沟通。
- `growth_potential`：从学习迁移、复杂问题处理、主动负责、复盘迭代、责任范围扩大等维度推理成长性。
- `objective_experiences`：客观工作或实习经历。
- `project_experiences`：项目经历、角色、技术栈、成果和原文证据。
- `suitable_roles`：适合岗位推荐，并包含成长适配分和成长适配理由。
...
这部分是当前答辩展示页新增强调的能力，用于说明系统不是简单地“生成面试题”，而是在面试前、中、后都加入了推理和验证机制。核心文件包括：

- `app/services/resume/llm_agent.py`
- `app/services/interview/growth_analyzer.py`
- `app/services/interview/scenario_engine.py`
- `app/services/interview/reasoning_engine.py`
- `app/services/interview/evaluation_engine.py`
- `app/services/interview/quality_validator.py`

成长性推理：

- 简历语义分析会输出 `growth_potential`，包含总体成长性、成长等级、支撑证据和分维度判断。
- 推理框架覆盖学习迁移、复杂问题处理、主动负责、复盘迭代、责任范围扩大/难度提升。
- 岗位推荐结果会补充 `growth_fit_score` 和 `growth_fit_reason`，说明候选人的成长轨迹是否适合目标岗位。
- `GrowthAnalyzer` 可结合岗位画像、候选人经历和外部信号生成非量化的成长性分析，强调叙事判断而不是单纯分数。

需求场景引擎：

- `ScenarioEngine` 内置技术深耕、技术拓展、团队管理、质量提升、业务创新等场景模板。
- 场景会把公司“当前缺什么能力”转换为面试关注点，使问题生成不只依赖岗位名称。
- 场景信息会与画像信号维度、必须验证项和风险画像共同进入面试策略。

实时推理链：

- `InterviewReasoner` 在每轮回答后更新 `reasoning_log`、`dimension_states` 和 `claimed_facts`。
- 系统会从逻辑性、具体性、反思性、主动性评估回答质量。
- 如果回答缺少细节、出现跨轮不一致或需要真实性验证，系统会把下一步动作标记为 `deepen`、`probe_risk` 或 `verify_authenticity`。
- 面试引擎根据推理结果决定继续深挖、推进到下一个维度、风险验证或收尾。

质量验证：

- `NarrativeEvaluationEngine` 输出叙事化评估报告，要求判断绑定原始问答证据。
- `JudgmentQualityValidator` 对最终评估做证据链、稳定性和区分度三层检查。
- 无依据判断、证据较弱判断、跨次判断波动或维度区分度不足时，会生成人工复核提示。
...
- GitHub、博客、适合岗位、多维履历画像、成长性推理和辅助问题写入 `external_profiles`。
...
- 岗位名称、部门、职级、薪资范围。
- 学历、年限、技能和软技能要求。
- 需求场景，例如技术深耕、技术拓展、团队管理、质量提升或业务创新。
- 画像信号维度。
- 必须验证项。
- 加分信号。
- 风险画像。
...
- `verification_dimensions`：需要验证的能力维度计划。
- `dimension_id`：验证维度编号。
- `criteria`：该维度的通过标准。
- `success_signals` / `risk_signals`：正向信号和风险信号。
- `probing_strategies`：回答模糊时的追问策略。
- `sections`：面试环节列表。
- `section_name`：环节名称。
- `duration_minutes`：环节时长。
- `focus_area`：考察重点。
- `linked_dimensions`：该环节关联的验证维度。
- `questions`：问题列表。
- `question_id`：问题编号。
- `question_text`：问题内容。
...
- `overview`：一句话总结候选人的关键底色。
- `dimension_reports`：按画像维度输出信号等级、判断、推理过程、盲区和证据。
- `risks`：风险点、严重程度、证据或待验证原因。
- `overall_judgment`：推荐结论、最强信号、最担忧信号和半年后预期。
- `system_feedback`：信号充分/不足维度和下次问题设计建议。
- `quality_validation`：证据链、稳定性、区分度和人工复核提示。
- `strengths` / `weaknesses`：候选人优势和不足。
- `ai_comment`：综合评价。
...
系统不仅输出候选人“当前会什么”，还通过 `growth_potential`、多维履历画像和岗位成长适配解释“未来能不能长到岗位需要的位置”。最终评估还会经过证据链、稳定性和区分度验证，使 AI 判断更适合被质疑、复核和改进。
...
- 将模糊招聘需求结构化。
- 将简历内容拆解为可验证证据。
- 将候选人成长性、职业倾向和岗位成长适配纳入解释。
- 将面试问题绑定岗位画像和候选人经历。
- 将每轮回答沉淀为推理日志和维度验证状态。
- 将面试过程完整记录并可复盘。
- 将最终评估进行证据链、稳定性和区分度验证。
