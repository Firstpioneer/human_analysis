...
class ProfileManager {
  constructor() {
    this.profile = null;
    this.getMessages = null; // callback to get chat messages
  ...
  updateProfile(profile) {
    console.log('[DEBUG ProfileManager] updateProfile 被调用, profile:', profile ? '有数据' : 'null');
    if (!profile) {
      console.trace('[DEBUG ProfileManager] 调用堆栈:');
    }
    this.profile = profile;
    this._render();
    console.log('[DEBUG ProfileManager] _render 完成, contentEl innerHTML 长度:', this.contentEl.innerHTML.length);
  }
    ...
    if (this.profile.job_title || this.profile.company_context) {
      html += this._renderSection('基本信息', 'info', () => {
        ...
        if (this.profile.job_title) {
          inner += `<div class="context-item">
            <div class="context-label">岗位名称</div>
            <div class="context-value">${this._escape(this.profile.job_title)}</div>
          </div>`;
        }
        const ctx = this.profile.company_context || {};
        if (ctx.why_hire) {
          inner += `<div class="context-item">
            <div class="context-label">招聘原因</div>
            <div class="context-value">${this._escape(ctx.why_hire)}</div>
          </div>`;
        }
        if (ctx.team_description) {
          inner += `<div class="context-item">
            <div class="context-label">团队现状</div>
            <div class="context-value">${this._escape(ctx.team_description)}</div>
          </div>`;
        }
        if (ctx.business_context) {
          inner += `<div class="context-item">
            <div class="context-label">业务场景</div>
            <div class="context-value">${this._escape(ctx.business_context)}</div>
          </div>`;
        }
        return inner;
      });
          ...
          <div class="role-card">
            ...
            ${role.key_responsibilities && role.key_responsibilities.length > 0 ? `
              <ul class="role-responsibilities">
                ${role.key_responsibilities.map(r => `<li>${this._escape(r)}</li>`).join('')}
              </ul>
            ` : ''}
          </div>
        `).join('');
      ...
      html += this._renderSection('任职条件', 'conditions', () => {
        ...
        if (this.profile.must_have && this.profile.must_have.length > 0) {
          inner += `<div class="context-label" style="margin-bottom:8px;">必要条件</div>
            <div class="profile-tags">
              ${this.profile.must_have.map(t => `<span class="profile-tag must-have">${this._escape(t)}</span>`).join('')}
            </div>`;
        }
        if (this.profile.nice_to_have && this.profile.nice_to_have.length > 0) {
          inner += `<div class="context-label" style="margin-top:16px;margin-bottom:8px;">加分项</div>
            <div class="profile-tags">
              ${this.profile.nice_to_have.map(t => `<span class="profile-tag nice-to-have">${this._escape(t)}</span>`).join('')}
            </div>`;
        }
        return inner;
      });
    ...
    if (this.profile.anti_profile && this.profile.anti_profile.length > 0) {
      html += this._renderSection('排除条件', 'anti', () => {
        return `<div class="profile-tags">
          ${this.profile.anti_profile.map(t => `<span class="profile-tag anti">${this._escape(t)}</span>`).join('')}
        </div>`;
      });
    }
  ...
  _renderSection(title, icon, contentFn) {
    const iconSvg = this._getSectionIcon(icon);
    return `
      <div class="profile-section">
        <div class="profile-section-header">
          <span class="profile-section-icon">${iconSvg}</span>
  ...
  openEditor() {
    ...
    overlay.querySelector('#edit-cancel-btn').addEventListener('click', () => {
      overlay.remove();
    });
